...
<div class="row">
  ...
  <div class="col-md-3">
    ...
    <div class="alert-text">
      <b> TOTAL RW </b>, <br>
      <span class="text-muted"> {{ \DB::table('tps_location_rws')->get()->count(); }} </span>
    </div>
  </div>
</div>
<div class="col-md-3">
  ...
  <div class="alert-text">
    <b> TOTAL RT </b>, <br>
    <span class="text-muted"> {{ \DB::table('tps_location_rts')->get()->count(); }} </span>
  </div>
</div>
</div>
</div>
    ...
    <div class="alert alert-custom alert-white alert-shadow fade show gutter-b" role="alert">
      ...
      <div class="alert-text">
        ...
        <center class="font-weight-bold">
          - AREA 1 - <br> 129 Participant
        </center>
        <hr>

        <table width="100%" class="table table-bordered table-hover" id="exilednoname" style="width: 100%;">
          <thead>
            <th> KELURAHAN </th>
          ...
          <tbody>
            <tr>
              @php
              $rt = 0; $rw = 0; for ($i = 1; $i <= 6; $i++) {
                $rw += \DB::table('tps_location_rws')->where('id_village', $i)->get()->count();
                $rt += \DB::table('tps_location_rts')->where('id_village', $i)->get()->count();
              }
              @endphp
              <td> ANDIR </td>
              ...
              <div class="col-md-3">
                <div class="alert alert-custom alert-white alert-shadow fade show gutter-b" role="alert">
                  ...
                  <div class="alert-text">
                    @php
                    $rw = 0; $rt = 0;
                    for ($i = 1; $i <= 6; $i++) { $rw += \DB::table('tps_location_rws')->where('id_village', $i)->get()->count(); }
                    for ($i = 1; $i <= 6; $i++) { $rt += \DB::table('tps_location_rts')->where('id_village', $i)->get()->count(); }
                    @endphp
                    <b> ANDIR </b>,
                    <span class="text-muted"> {{ $rw }} RW </span> - <span class="text-muted"> {{ $rt }} RT </span>

                    @php
                    $rw = 0; $rt = 0;
                    for ($i = 7; $i <= 12; $i++) { $rw += \DB::table('tps_location_rws')->where('id_village', $i)->get()->count(); }
                    for ($i = 7; $i <= 12; $i++) { $rt += \DB::table('tps_location_rts')->where('id_village', $i)->get()->count(); }
                    @endphp
                    <b> CICENDO </b>,
                    <span class="text-muted"> {{ $rw }} RW </span> - <span class="text-muted"> {{ $rt }} RT </span>
                  </div>
